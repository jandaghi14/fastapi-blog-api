...
async def test_create_tag_already_exists(client: AsyncClient, auth_header):
    ...
    assert response.json()[
        'detail'] == f"Tag with name 'test_tag' already exist."
...
async def test_get_all_tag(client: AsyncClient, auth_header, create_tag):
    header = await auth_header()
    await create_tag(header=header)
    await create_tag(header=header)
    await create_tag(header=header)
    ...
    response = await client.get(f"/tag/get_all", headers=header)
    assert len(response.json()) == 4
    assert response.status_code == 200


async def test_get_all_tag_not_found(client: AsyncClient, auth_header, create_tag):
    header = await auth_header()
...
async def test_update_success(client: AsyncClient, auth_header, create_tag):
    ...
    response = await client.put(f"/tag/update",
